...
from llm_orchestrator.exceptions import ModelUnavailableError, ProviderError
...
class OllamaProvider(BaseProvider):
    ...
    def _get_model_capabilities(
        ...
        has_vision = any(keyword in name_lower for keyword in ["vision", "llava", "bakllava"])
        
        # Most modern models support function calling (llama3.1+, mistral, etc.)
        has_function_calling = any(keyword in name_lower for keyword in [
            "llama3.1", "llama3.2", "llama3.3", "mistral", "qwen"
        ])
    ...
    def _build_structured_prompt(self, prompt: str, response_schema: type) -> str:
        """
        Build a prompt that instructs the model to return structured JSON.
        
        Args:
            prompt: Original prompt
            response_schema: Pydantic model defining the expected structure
