...
class OllamaProvider(BaseProvider):
    ...
    def generate_text(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if e.response.status_code == 404:
                raise ModelUnavailableError(
                    model=model_name, provider="ollama", reason="Model not found"
                )
            raise ProviderError(
                provider="ollama",
    ...
    def _get_model_capabilities(
        ...
        has_vision = any(
            keyword in name_lower for keyword in ["vision", "llava", "bakllava"]
        )

        # Most modern models support function calling (llama3.1+, mistral, etc.)
        has_function_calling = any(
            keyword in name_lower
            for keyword in ["llama3.1", "llama3.2", "llama3.3", "mistral", "qwen"]
        )
