...
from google.genai.errors import ClientError, APIError as GeminiAPIError
...
class GeminiProvider(BaseProvider):
    ...
    def _get_model_capabilities(self, model_name: str) -> ModelCapabilities:
        ...
        is_gemini_2 = any(model_name.startswith(m.replace("-exp", "")) for m in self.GEMINI_2_MODELS)
        is_gemini_15 = any(model_name.startswith(m.replace("-latest", "")) for m in self.GEMINI_15_MODELS)
        is_gemini_10 = any(model_name.startswith(m.replace("-latest", "")) for m in self.GEMINI_10_MODELS)
