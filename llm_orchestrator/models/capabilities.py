...
from llm_orchestrator.types import ModelDetails, CapabilityRequirements
...
def matches_requirements(
    ...
    if requirements.min_context_window:
        if not caps.min_context_window or caps.min_context_window < requirements.min_context_window:
            return False
...
def get_capability_summary(models: List[ModelDetails]) -> dict:
    ...
    return {
        ...
        "with_function_calling": sum(1 for m in models if m.capabilities.function_calling),
        "max_context_window": max(
            (m.capabilities.min_context_window for m in models if m.capabilities.min_context_window),
            default=0
        ),
        "avg_context_window": (
            sum(m.capabilities.min_context_window for m in models if m.capabilities.min_context_window) // 
            sum(1 for m in models if m.capabilities.min_context_window)
        ) if any(m.capabilities.min_context_window for m in models) else 0,
...
def compare_capabilities(model1: ModelDetails, model2: ModelDetails) -> dict:
    ...
    return {
        ...
        "model1_score": score1,
        "model2_score": score2,
        "better_model": model1.name if score1 > score2 else (model2.name if score2 > score1 else "tie"),
        "differences": {
