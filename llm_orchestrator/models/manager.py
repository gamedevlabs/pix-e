...
from llm_orchestrator.config import Config, get_config
from llm_orchestrator.exceptions import (
    ModelUnavailableError,
    ProviderError,
)
from llm_orchestrator.models.providers import (
    BaseProvider,
    OllamaProvider,
    OpenAIProvider,
    GeminiProvider,
...
class ModelManager:
    ...
    def _init_providers(self) -> None:
        ...
        if not self.providers:
            raise ProviderError(
                message="No LLM providers available. Configure at least one provider (Ollama, OpenAI, or Gemini).",
                provider="none"
            )
        
        logger.info(f"📊 Initialized {len(self.providers)} provider(s): {', '.join(self.providers.keys())}")
    ...
    def list_models(self, refresh: bool = False) -> List[ModelDetails]:
        ...
        if (not refresh and 
            ...
            now - self._cache_timestamp < self.config.cache_ttl_seconds):
    ...
    def generate_auto(
        ...
        **kwargs: Any
        ...
        # Select best model
        model = self.select_model(requirements, model_preference)
        provider = self.providers[model.provider]
