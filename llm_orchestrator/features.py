...
class FeatureID(str, Enum):
    """
    Registered features in pix:e. These are the main features that use LLM orchestration.
    """
...
class OperationRegistry:
    ...
    _operations: Dict[Tuple[str, str], OperationMetadata] = {
        ...
        (FeatureID.PILLARS, PillarsOperations.EVALUATE_CONSISTENCY): OperationMetadata(
            id=PillarsOperations.EVALUATE_CONSISTENCY,
            feature_id=FeatureID.PILLARS,
            description="Check multiple pillars for contradictions and alignment",
            request_schema_uri="store://schemas/pillars.evaluate_consistency@1.0.0.req.json",
            response_schema_uri="store://schemas/pillars.evaluate_consistency@1.0.0.res.json",
        ),
        
        # ============================================
        # SPARC Feature
        # ============================================
        (FeatureID.SPARC, SPARCOperations.EVALUATE_CONCEPT): OperationMetadata(
            ...
            request_schema_uri="store://schemas/sparc.evaluate_concept@1.0.0.req.json",
            response_schema_uri="store://schemas/sparc.evaluate_concept@1.0.0.res.json",
        ),
        ...
        # Moodboards Feature
        # ============================================
        (FeatureID.MOODBOARDS, MoodboardsOperations.GENERATE_IMAGES): OperationMetadata(
            id=MoodboardsOperations             .GENERATE_IMAGES,
            feature_id=FeatureID.MOODBOARDS,
            description="Generate images from a text prompt",
            request_schema_uri="store://schemas/moodboards.generate_images@1.0.0.req.json",
            response_schema_uri="store://schemas/moodboards.generate_images@1.0.0.res.json",
        ),
        (FeatureID.MOODBOARDS, MoodboardsOperations.SUGGEST_VARIATIONS): OperationMetadata(
            id=MoodboardsOperations.SUGGEST_VARIATIONS,
            feature_id=FeatureID.MOODBOARDS,
            description="Suggest prompt variations for exploration",
            request_schema_uri="store://schemas/moodboards.suggest_variations@1.0.0.req.json",
            response_schema_uri="store://schemas/moodboards.suggest_variations@1.0.0.res.json",
        ),
        (FeatureID.MOODBOARDS, MoodboardsOperations.ALIGN_WITH_PILLARS): OperationMetadata(
            id=MoodboardsOperations.ALIGN_WITH_PILLARS,
            feature_id=FeatureID.MOODBOARDS,
            description="Evaluate how well generated images align with design pillars",
            request_schema_uri="store://schemas/moodboards.align_with_pillars@1.0.0.req.json",
            response_schema_uri="store://schemas/moodboards.align_with_pillars@1.0.0.res.json",
        ),
    }
    ...
    def get(cls, feature: str, operation: str) -> OperationMetadata:
        ...
        from llm_orchestrator.exceptions import UnknownFeatureError, UnknownOperationError
    ...
    def list_operations(cls, feature: Optional[str] = None) -> List[OperationMetadata]:
        ...
        if feature:
            return [
                op for (f, _), op in cls._operations.items() 
                if f == feature
            ]
