...
class UserContext(BaseModel):
    ...
    api_tokens: Optional[Dict[str, str]] = Field(
        None, description="User-specific API tokens (orchestrator may ignore)"
    )
...
class ResponseMetadata(BaseModel):
    ...
    operation_schema: Optional[str] = Field(
        None, description="Operation schema used (e.g., 'sparc.evaluate_concept@1.0.0')"
    )
...
class ErrorInfo(BaseModel):
    ...
    diagnostics: Optional[Dict[str, Any]] = Field(
        None, description="Additional diagnostic info (trace_id, suggested_fixes)"
    )
