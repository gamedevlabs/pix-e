...
const confusionPaletteBase = [
  '#27599e', '#a1d5cc', '#d9c85f',
  '#3b6cb2', '#69a89f', '#f8ef9a',
  '#153b7a', '#8ac9c0', '#f2e686',
  '#27599e', // repeat ok
]

export function usePlayerExpectationCharts(baseUrl = 'http://localhost:8000/api') {
  // reactive chart state
  const aspectChartData        = ref<any>(null)
  const sentimentChartData     = ref<any>(null)
  const lineChartData          = ref<any>(null)
  const sentimentPieData       = ref<any>(null)
  const heatmapData            = ref<any>(null)
  const topConfusionsChartData = ref<any>(null)
