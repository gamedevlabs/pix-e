...
draws a horizontal-scrollable bar chart for the most mentioned aspects
bar height = total mentions for that aspect (relative to the max)
bar is stacked by sentiment buckets (positive/neutral/negative/missing).
on hover, it shows a custom tooltip with detailed counts + net sentiment. -->
...
// Take only the first N rows (the "top" entries) default 10 but using 15 atm
const top = computed(() => {
  const xs = props.rows ?? []
  const n = props.maxBars ?? 10
  return xs.slice(0, n)
})
...
// Text shown in the hover tooltip for one bar
...
const plotMinWidth = computed(() => {
  const n = top.value.length || (props.maxBars ?? 10)
...
// Show tooltip on hover + set its text, then position it near the cursor
...
<template>
  <div class="space-y-2">
    ...
    <!-- Two-column layout: left = y-axis labels, right = scrollable plot -->
