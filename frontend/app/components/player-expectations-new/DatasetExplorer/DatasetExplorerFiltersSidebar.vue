 ...
 *
 *  props are plain values (Sets/Arrays/strings)
 *  search inputs use v-model emitters (no prop mutation)
 */
...
const props = defineProps<{
  ...
  selectedAestheticCodes: Set<number>
  selectedFeatureCodes: Set<number>
  selectedPainCodes: Set<number>
...
type Emits = {
  'update:featureSearch': [string]
  'update:painSearch': [string]
}

const emit = defineEmits<Emits>()

const featureSearchModel = computed({
  get: () => props.featureSearch,
  set: (v: string) => emit('update:featureSearch', v),
})

const painSearchModel = computed({
  get: () => props.painSearch,
  set: (v: string) => emit('update:painSearch', v),
})

function hasInSet<T>(s: Set<T>, value: T): boolean {
  return s.has(value)
}
      ...
      <div class="space-y-6">
        ...
        <div>
          ...
          <UAccordion class="mt-3" :items="[{ label: 'Game Features', slot: 'features' }]">
            <template #features>
              <div class="mt-2 space-y-3">
                <UInput v-model="featureSearchModel" size="sm" placeholder="Quick search codes…" />
                <div class="space-y-3">
                  <div
                    v-for="g in filteredFeatureGroups"
                    :key="g.top"
                  ...
                  >
                    ...
                    <div v-if="g.subs.length" class="mt-2 flex flex-col gap-1">
                      <UButton
                        ...
                        @click="clickCode('feature', s.code)"
                      >
                        {{ s.label }}
                      </UButton>
                    </div>
          ...
          <UAccordion class="mt-3" :items="[{ label: 'Pain Points', slot: 'pain' }]">
            <template #pain>
              <div class="mt-2 space-y-3">
                <UInput v-model="painSearchModel" size="sm" placeholder="Quick search codes…" />
                <div class="space-y-3">
                  <div
                    v-for="g in filteredPainGroups"
                    :key="g.top"
                    class="border rounded p-2 border-slate-200 dark:border-slate-800"
                  >
                    <div class="flex items-center justify-between gap-2">
