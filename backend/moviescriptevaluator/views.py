...
class MovieProjectView(viewsets.ModelViewSet):
    ...
    def analyze_movie_script(self, request, pk):
        ...
        get_llm_connector().logger.write_log("The analyze script action has been triggered with -> script_id: {}, model_name: {}".format(script_id, model_name))
    ...
    def get_recommendations(self, request, pk):
        ...
        get_llm_connector().logger.write_log(
            "The get recommendation action has been triggered with -> model_name: {}".format(model_name))
        ...
        if len(response.result) != 0:
            llm_connector = get_llm_connector()
            llm_connector.logger.write_log("Number of hallucinated items: {}, percentage of hallucinated items: {}".format(hallucinated_items, hallucinated_items / len(response.result)))
            llm_connector.logger.write_log("Total number of items generated by LLM: {}".format(len(response.result)))

        try:
            self.evaluate_missing_items(pk)
        except Exception as e:
            print(e)
            get_llm_connector().logger.write_log("Failed to evaluate missing items: {}".format(e))
    ...
    def missing_items(self, request, pk):
        ...
        get_llm_connector().logger.write_start_action()
        get_llm_connector().logger.write_log(
            "The missing items action has been triggered with -> model_name: {}".format(model_name))
