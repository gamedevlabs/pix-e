...
class TGIAPIService(BaseLLMService):
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            self.is_loaded = True
            logger.info(f"TGI client initialized for {self.endpoint} (connection test skipped)")
            return True
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info("Model is loading (503) - this is normal for cold start")
                    return True  # Model is loading but will be available
                else:
                    logger.warning(f"Model endpoint returned {response.status_code}")
                    return response.status_code < 500  # Client errors are ok, server errors are not
                    
            except requests.exceptions.Timeout:
                logger.warning("Model endpoint test timed out - assuming available but slow")
