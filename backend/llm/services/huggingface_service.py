...
class HuggingFaceLLMService(BaseLLMService):
    ...
    def get_suggestions(self, prompt: str, num_suggestions: int = 3, **kwargs) -> List[str]:
        """Generate multiple text suggestions for creative prompts"""
        if not self.is_loaded:
            raise LLMServiceError("Model not loaded. Call load_model() first.")
        
        try:
            suggestion_type = kwargs.get('suggestion_type', 'short')  # Handle suggestion_type parameter
                ...
                try:
                    ...
                    for output in outputs:
                        ...
                        if generated_text.startswith(template):
                            ...
                            if suggestion and len(suggestion.split()) >= 2:  # At least 2 words
                                # Create final suggestion
                                if template.endswith(', '):
                                    final_suggestion = f"{prompt}, {suggestion}"
                                elif 'scene with' in template:
                                    final_suggestion = f"A {prompt} scene with {suggestion}"
                                elif 'featuring' in template:
                                    final_suggestion = f"{prompt} featuring {suggestion}"
            ...
            while len(all_suggestions) < num_suggestions:
                fallback = self._generate_fallback_suggestion(prompt, len(all_suggestions))
