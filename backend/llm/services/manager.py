...
class LLMServiceManager:
    ...
    def load_service(self, service_id: str, model_name: str = None, user_token: str = None):
    ...
    def _create_contextual_prompt(self, user_input: str, mode: str, num_suggestions: int, suggestion_type: str = 'short') -> str:
        ...
        if suggestion_type.lower() == 'long':
            ...
            format_instruction = "Format as complete descriptive sentences, one per line:"
        else:
...
{format_instruction}"""
    ...
    def get_suggestions(self, prompt: str, service_id: str = None, num_suggestions: int = 3, mode: str = 'default', user=None, **kwargs) -> List[str]:
        ...
        if user and service_id:
            ...
            if not user_token:
                ...
                logger.error(f"No token found for service {service_id} ({service_name}) for user {user.username}")
                raise LLMServiceError(f"API token required for {service_name}. Please configure your API token in the AI suggestions panel.")
        ...
        if service_id and service_id.startswith('tgi_'):
            try:
                ...
                if service and service.is_model_loaded():
                    contextual_prompt = self._create_contextual_prompt(prompt, mode, num_suggestions, suggestion_type)
                    ...
                    for suggestion in suggestions:
                        if suggestion and len(suggestion.strip()) > 3:
                            ...
                            if (suggestion_text.startswith('SSH tunnel') or 
                                suggestion_text.startswith('TGI') or
                                suggestion_text.startswith('Please') or
                                suggestion_text.startswith('Here are') or
                                suggestion_text.startswith('I can provide')):
                                continue
                            ...
                            if suggestion_type == 'short':
                                ...
                                prefixes_to_remove = [
                                    'A series of', 'A collage art piece', 'An abstract painting',
                                    'A painting of', 'A drawing of', 'An image of'
                                ]
                                for prefix in prefixes_to_remove:
                                    if suggestion_text.lower().startswith(prefix.lower()):
                                        suggestion_text = suggestion_text[len(prefix):].strip()
                                        break
                                ...
                                if len(words) > 8:
                                    ...
                                    suggestion_text = ' '.join(words[:6])  # Take first 6 words
                    ...
                    if cleaned_suggestions:
                        ...
                        raise LLMServiceError(f"TGI service {service_id} failed to generate valid suggestions")
                    
            except Exception as e:
                logger.error(f"TGI service {service_id} failed: {e}")
                # Check if it's a timeout or rate limit error
                if "timed out" in str(e).lower() or "timeout" in str(e).lower():
                    raise LLMServiceError(f"The TGI service timed out after 30 seconds. Please check your Hugging Face token and try again.")
                elif "rate limit" in str(e).lower() or "429" in str(e):
                    raise LLMServiceError(f"Rate limit exceeded for TGI service. Please check your Hugging Face token or try again later.")
                else:
                    # Don't fall back - user specifically requested this service
                    raise LLMServiceError(f"The TGI service ({service_id}) failed to respond. Please check your Hugging Face token and try again.")
            ...
            try:
                ...
                if service and service.is_model_loaded():
                    ...
                    if cleaned_suggestions:
                        ...
                        raise LLMServiceError(f"GitHub Models service {service_id} failed to generate valid suggestions")
                    
            except Exception as e:
                logger.error(f"GitHub Models service {service_id} failed: {e}")
                # Check if it's a timeout or rate limit error  
                if "timed out" in str(e).lower() or "timeout" in str(e).lower():
                    raise LLMServiceError(f"The GitHub Models service timed out after 30 seconds. Please check your GitHub token and try again.")
                elif "rate limit" in str(e).lower() or "429" in str(e):
                    raise LLMServiceError(f"Rate limit exceeded for GitHub Models. Please check your GitHub token or try again later.")
                else:
                    # Don't fall back - user specifically requested this service
                    raise LLMServiceError(f"The GitHub Models service ({service_id}) failed to respond. Please check your API token and try again.")
        
        # If no specific service requested, require user to select one
        if not service_id:
            raise LLMServiceError("Please select an AI service from the dropdown menu to generate suggestions.")
    ...
    def _get_user_token(self, service_id: str, user) -> Optional[str]:
        ...
        if not AIServiceToken or not user or not user.is_authenticated:
            logger.warning(f"Token retrieval failed: AIServiceToken={bool(AIServiceToken)}, user={bool(user)}, authenticated={user.is_authenticated if user else False}")
            return None
        ...
        try:
            tokens = AIServiceToken.objects.filter(
                user=user,
                service_type=required_service_type,
                is_active=True
            )
