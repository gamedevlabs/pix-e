...
class GitHubModelsService(BaseLLMService):
    ...
    def generate_text(self, prompt: str, max_tokens: int = 100, temperature: float = 0.7, 
                     num_return_sequences: int = 3, suggestion_type: str = 'short') -> List[str]:
        """Generate text using GitHub Models API"""
        if not self.is_loaded or not self.client:
            raise LLMServiceError("GitHub Models client not loaded. Call load_model() first.")
...
{example_format}"""
            ...
            messages = [
                SystemMessage("You are a helpful assistant that provides creative, diverse suggestions for artistic prompts."),
            ...
            if response and response.choices:
                ...
                if generated_text and generated_text.strip():
                    ...
                    if len(suggestions) < num_return_sequences:
                        ...
                        all_text = generated_text.replace('\n', ' ').replace(',', '\n').replace(';', '\n')
                        for line in all_text.split('\n'):
                            line = line.strip()
                            if line and len(line) > 3 and len(line) < (100 if suggestion_type == 'long' else 50):
                                # Remove common prefixes
                                for prefix in ['with', 'featuring', 'including', 'showing']:
                                    if line.startswith(prefix + ' '):
                                        line = line[len(prefix) + 1:]
                                        break
                                
                                if line not in suggestions and len(suggestions) < num_return_sequences:
            ...
            raise LLMServiceError(f"Unable to generate suggestions for prompt: {prompt[:50]}...")
    ...
    def get_suggestions(self, prompt: str, max_tokens: int = 100, temperature: float = 0.7, 
                       num_suggestions: int = 3, **kwargs) -> List[str]:
        """Get text suggestions (alias for generate_text for compatibility)"""
        suggestion_type = kwargs.get('suggestion_type', 'short')
        return self.generate_text(prompt, max_tokens, temperature, num_suggestions, suggestion_type)
