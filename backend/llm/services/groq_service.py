...
class GroqService(BaseLLMService):
    ...
    def load_model(self) -> bool:
        ...
        try:
            if not self.api_key:
                logger.info("No GROQ_API_KEY found. Get free API key from: https://console.groq.com/")
                return False
            ...
            if self.model_name not in model_ids:
                logger.warning(f"Model {self.model_name} not available. Available: {model_ids[:5]}")
    ...
    def generate_suggestions(self, prompt: str, num_suggestions: int = 3, mode: str = 'default', suggestion_type: str = 'short', **kwargs) -> List[str]:
...
Return only the suggestions, one per line, no numbers or bullets:"""
            ...
            response_text = self.generate_text(instruction, max_length=150, temperature=0.8)
            ...
            if not suggestions:
                ...
                if suggestion_type == 'long':
                    ...
                    if mode == 'gaming':
                        suggestions = ["cinematic lighting", "epic atmosphere", "detailed textures"]
                    else:
                        suggestions = ["vibrant palette", "artistic texture", "modern composition"]
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'description': self.AVAILABLE_MODELS.get(self.model_name, {}).get('description', 'Groq model'),
            'size': self.AVAILABLE_MODELS.get(self.model_name, {}).get('size', 'Unknown'),
            'free_tier': True
        }
