        ...
        super().__init__(
            ...
            suggestion=(
                (
                    ...
                    f"{', '.join(str(f) for f in available_features)}"
                )
                if available_features
                else None
            ),
...
class UnknownOperationError(OrchestratorError):
    ...
    def __init__(
        ...
        if available_operations is None:
            try:
                from llm.features import list_operations
...
class RunNotFoundError(OrchestratorError):
    ...
    def __init__(self, run_id: str):
        super().__init__(
            ...
            suggestion=(
                "Check that the run_id is correct " "and the run hasn't expired"
            ),
        )
...
class ModelUnavailableError(OrchestratorError):
    ...
    def _get_default_suggestion(self, provider: str, model: str) -> str:
        ...
        if provider == "ollama":
            return f"Ensure Ollama is running " f"and run 'ollama pull {model}'"
