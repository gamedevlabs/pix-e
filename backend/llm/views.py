    ...
    if sd_model is None:
        try:
            ...
            try:
                ...
                logger.info(
                    "Successfully loaded standard Stable Diffusion model on CUDA"
                )
            except Exception as e2:
                logger.error(
                    f"Failed to load Stable Diffusion: {str(e2)}", exc_info=True
                )
...
def generate_gaming_images(prompt, num_images=3):
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("🎨 STABLE DIFFUSION IMAGE GENERATION REQUEST")
        ...
        for i in range(num_images):
            ...
            seed = base_seed + i
            logger.info(
                f"Generating image {i+1}/{num_images} with seed {seed} "
                f"and prompt: '{prompt}'..."
            )
            ...
            if (
                ...
                raise Exception(
                    f"Stable Diffusion pipeline returned no images for image {i+1}"
                )
        ...
        for i, img in enumerate(images):
            try:
                ...
                if not os.path.exists(img_path):
                    raise Exception(
                        f"Failed to save image {i+1} to disk - "
                        f"file does not exist after save"
                    )

                file_size = os.path.getsize(img_path)
                if file_size < 1000:  # Image should be at least 1KB
                    raise Exception(
                        f"Image {i+1} file size too small "
                        f"({file_size} bytes), generation may have failed"
                    )

                logger.info(
                    f"✅ Saved image {i+1}/{num_images}: {img_path} ({file_size} bytes)"
                )
                image_urls.append(f"/media/moodboard_{img_id}.png")

            except Exception as img_error:
                logger.error(
                    f"❌ Failed to process image {i+1}: {str(img_error)}", exc_info=True
                )
                # Re-raise to fail the entire generation
                raise Exception(
                    f"Failed to process image {i+1}/{num_images}: {str(img_error)}"
                )

        logger.info(
            f"Successfully generated and saved {len(image_urls)} images "
            f"with Stable Diffusion"
        )

        # Final verification
        if len(image_urls) != num_images:
            raise Exception(
                f"Generated {len(image_urls)} URLs but expected {num_images}"
            )
        ...
        logger.warning(
            "ERROR: Falling back to placeholder images - "
        ...
        raise Exception(
            ...
            f"Please check that Stable Diffusion is properly "
            f"configured and running."
        )
