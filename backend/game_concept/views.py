...
class ProjectViewSet(ModelViewSet):
    ...
    def clone(self, request: Request, pk: Optional[int] = None) -> Response:
        ...
        if include_nodes:
            definitions = PxComponentDefinition.objects.filter(
                owner=source_project.user
            )
            ...
            nodes = PxNode.objects.filter(owner=source_project.user)
            for node in nodes:
        ...
        if include_charts:
            charts = PxChart.objects.filter(owner=source_project.user)
            for chart in charts:
