...
class SentimentData(APIView):
    def get(self, request):
        expectation_type = request.query_params.get(
            "type", "explicit"
        )  # default to explicit
        ...
        def load_csv(path, drop_cols, replace_explicit=False):
            if os.path.exists(path):
                ...
                if replace_explicit and "explicit_expectations" in df.columns:
                    ...
                    df.rename(
                        columns={"explicit_expectations": "expectations"}, inplace=True
                    )
        ...
        print(
            f"Checking explicit_path: {explicit_path}, exists: {os.path.exists(explicit_path)}"
        )
        print(
            f"Checking implicit_path: {implicit_path}, exists: {os.path.exists(implicit_path)}"
        )

        # Load datasets
        df_explicit = load_csv(
            explicit_path, drop_columns_explicit, replace_explicit=True
        )
        ...
        print(
            f"df_explicit shape: {df_explicit.shape}, columns: {df_explicit.columns.tolist()}"
        )
        print(
            f"df_implicit shape: {df_implicit.shape}, columns: {df_implicit.columns.tolist()}"
        )
        print(f"df_all shape: {df_all.shape}, columns: {df_all.columns.tolist()}")

        # Define condition for 'not_assigned' reviews
        not_assigned_condition = (
            df_all["expectations"].isnull()
            | (df_all["expectations"] == "")
            | (df_all["expectations"] == "[]")
        )
        ...
        if expectation_type == "explicit":
            df = df_all[
                (df_all["expectation_type"] == "explicit") & ~not_assigned_condition
            ].copy()
        elif expectation_type == "implicit":
            df = df_all[
                (df_all["expectation_type"] == "implicit") & ~not_assigned_condition
            ].copy()
        ...
        # Serialize to JSON
        json_str = df.to_json(
            orient="records", force_ascii=False, date_format="iso", double_precision=15
        )
