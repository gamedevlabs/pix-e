...
class ImageEditor:
    ...
    def load_image(self):
        ...
        try:
            # Check if it's an absolute path (for testing) or relative
            # path (for production)
        ...
        if filter_name == "blur":
            ...
            if intensity > 0:
                filter_obj = ImageFilter.UnsharpMask(
                    radius=2, percent=int(intensity * 150), threshold=3
                )
    ...
    def resize(
        self, width: int, height: int, maintain_aspect: bool = True
    ) -> "ImageEditor":
    ...
    def save_edited_image(self, original_filename: str) -> str:
        ...
        save_image.save(
            output, format=format_map[file_extension], quality=95, optimize=True
        )
        output.seek(0)
...
def apply_batch_edits(
    image_path: str, edits: Dict[str, Any]
