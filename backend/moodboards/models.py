...
class MoodboardManager(models.Manager):
    ...
    def get_user_moodboards(self, user):
        ...
        return self.filter(models.Q(user=user) | models.Q(shared_with=user)).distinct()
    ...
    def get_active_drafts(self, user, max_age_days=7):
        ...
        return self.filter(user=user, status="draft", updated_at__gte=cutoff_date)
...
class Moodboard(models.Model):
    ...
    is_public = models.BooleanField(
        default=False, db_index=True, help_text="Make this moodboard publicly viewable"
    )
    ...
    last_accessed = models.DateTimeField(
        default=timezone.now, help_text="Last time this moodboard was accessed"
    )
    ...
    def increment_view_count(self):
        ...
        Moodboard.objects.filter(id=self.id).update(
            view_count=models.F("view_count") + 1, last_accessed=timezone.now()
        )
...
class MoodboardImage(models.Model):
    ...
    image_url = models.CharField(
        max_length=1000, help_text="URL or path to the full-size image"
    )
    thumbnail_url = models.CharField(
        max_length=1000, blank=True, help_text="URL or path to optimized thumbnail"
    )
    original_filename = models.CharField(
        max_length=255, blank=True, help_text="Original filename if uploaded"
    )

    # Content and metadata
    title = models.CharField(
        max_length=255, blank=True, help_text="Custom title for the image"
    )
    description = models.TextField(
        blank=True, max_length=1000, help_text="Description or notes about the image"
    )
    ...
    generation_time = models.FloatField(
        null=True, blank=True, help_text="Time taken to generate image in seconds"
    )
    ...
    format = models.CharField(
        max_length=10, blank=True, help_text="Image format (PNG, JPG, etc.)"
    )
    ...
    style_tags = models.JSONField(
        blank=True, null=True, help_text="Automatically detected style characteristics"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(order_index__gte=0), name="image_order_index_positive"
            ),
        ]

    def __str__(self):
        title = (
            self.title or self.prompt[:50]
            if self.prompt
            else f"Image {str(self.id)[:8]}"
        )
    ...
    def display_title(self):
        ...
        return (
            self.title
            or (self.prompt[:50] + "..." if len(self.prompt) > 50 else self.prompt)
            or f"Image {str(self.id)[:8]}"
        )
    ...
    def select(self, order_index=None):
        ...
        if order_index is not None:
            ...
            max_order = MoodboardImage.objects.filter(
                moodboard=self.moodboard, is_selected=True
    ...
    def _reorder_siblings(self, old_index, new_index):
        ...
        if new_index > old_index:
            ...
            siblings.filter(
                order_index__gt=old_index, order_index__lte=new_index
            ).update(order_index=models.F("order_index") - 1)
        else:
            # Moving up - shift others down
            siblings.filter(
                order_index__gte=new_index, order_index__lt=old_index
            ).update(order_index=models.F("order_index") + 1)
...
class MoodboardTextElement(models.Model):
    ...
    def __str__(self):
        ...
        return f"Text: {content_preview} in {self.moodboard.title}"
...
class MoodboardShare(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, help_text="User receiving access"
    )
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="Optional expiration time for access"
    )
...
class MoodboardComment(models.Model):
    ...
    content = models.TextField(
        max_length=2000, help_text="Comment content (max 2000 characters)"
    )
    ...
    hidden_at = models.DateTimeField(
        null=True, blank=True, help_text="When comment was hidden"
    )
    ...
    def clean(self):
        ...
        if self.parent:
            ...
            if depth >= 3:
                raise ValidationError(
                    {"parent": "Comments can only be nested 3 levels deep"}
                )
...
def auto_cleanup_old_drafts(sender, instance, created, **kwargs):
    ...
    if created and instance.status == "draft":
        ...
        old_drafts = (
            Moodboard.objects.filter(user=instance.user, status="draft")
...
class MoodboardTemplate(models.Model):
    ...
    template_type = models.CharField(
        max_length=20, choices=TEMPLATE_TYPE_CHOICES, default="starter"
    )
    category = models.CharField(
        max_length=20, choices=Moodboard.CATEGORY_CHOICES, default="other"
    )
