...
class MoodboardImageInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['image_url', 'title', 'is_selected', 'order_index', 'source', 'created_at']
...
class MoodboardAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'user', 'category', 'status', 'image_count_display', 
        'is_public', 'created_at', 'updated_at'
    ]
    list_filter = ['category', 'status', 'is_public', 'created_at']
    search_fields = ['title', 'description', 'tags', 'user__username']
    readonly_fields = ['id', 'created_at', 'updated_at', 'image_count_display', 'selected_image_count_display']
    ...
    actions = ['make_public', 'make_private', 'archive_moodboards', 'activate_moodboards']
    
    def image_count_display(self, obj):
        """Display image count with link to images"""
        count = obj.image_count
        if count > 0:
            url = reverse('admin:moodboards_moodboardimage_changelist') + f'?moodboard__id__exact={obj.id}'
...
class MoodboardImageAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title_or_id', 'moodboard', 'source', 'is_selected', 
        'order_index', 'image_preview', 'created_at'
    ]
    ...
    fieldsets = [
        ('Basic Information', {
            'fields': ['id', 'moodboard', 'title', 'description']
        }),
        ('Image Data', {
            'fields': ['image_url', 'thumbnail_url', 'image_preview', 'original_filename']
        }),
        ('AI Generation', {
            'fields': ['prompt', 'generation_params'],
            'classes': ['collapse']
        }),
        ('Metadata', {
            'fields': ['source', 'tags', 'is_selected', 'order_index']
        }),
        ('Properties', {
    ...
    def image_preview(self, obj):
        ...
        if obj.thumbnail_url:
            return format_html('<img src="{}" width="100" height="100" style="object-fit: cover;" />', obj.thumbnail_url)
        elif obj.image_url:
...
class MoodboardTemplateAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'template_type', 'category', 'created_by', 
