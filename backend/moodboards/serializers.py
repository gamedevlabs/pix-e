...
class MoodboardImageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'prompt', 'generation_params', 'title', 'description', 
            'source', 'tags', 'tag_list', 'is_selected', 'order_index',
...
class MoodboardImageCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MoodboardImage
        fields = [
            'image_url', 'prompt', 'title', 'description', 
            'source', 'tags', 'is_selected', 'order_index'
        ]
        extra_kwargs = {
...
class MoodboardCommentSerializer(serializers.ModelSerializer):
    ...
    def get_replies(self, obj):
        ...
        if obj.replies.exists():
            return MoodboardCommentSerializer(obj.replies.all(), many=True, context=self.context).data
        return []
...
class MoodboardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'tags', 'tag_list', 'is_public', 'color_palette',
            'images', 'image_count', 'selected_image_count', 
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'images', 'image_count', 'selected_image_count', 
...
class MoodboardListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'title', 'description', 'category', 'status',
            'tags', 'tag_list', 'is_public',
            'images', 'image_count', 'selected_image_count', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'images', 'image_count', 'selected_image_count', 
            'tag_list', 'created_at', 'updated_at'
...
class SharedMoodboardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'tags', 'tag_list', 'is_public',
            'images', 'image_count', 'selected_image_count', 
            'permission', 'shared_by', 'shared_at',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'images', 'image_count', 'selected_image_count', 
            'tag_list', 'permission', 'shared_by', 'shared_at',
            'created_at', 'updated_at'
        ]
...
class MoodboardDetailSerializer(serializers.ModelSerializer):
    ...
    shared_with = MoodboardShareSerializer(many=True, read_only=True, source='moodboardshare_set')
    image_count = serializers.ReadOnlyField()
    selected_image_count = serializers.ReadOnlyField()
    tag_list = serializers.ReadOnlyField()
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'title', 'description', 'category', 'status',
            'tags', 'tag_list', 'color_palette', 'is_public',
            'images', 'selected_images', 'unselected_images', 'comments',
            'shared_with', 'image_count', 'selected_image_count',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'images', 'selected_images', 'unselected_images',
            'comments', 'shared_with', 'image_count', 'selected_image_count',
            'tag_list', 'created_at', 'updated_at'
        ]
    ...
    def get_selected_images(self, obj):
        ...
        return MoodboardImageSerializer(selected_images, many=True, context=self.context).data
    
    def get_unselected_images(self, obj):
        """Get only unselected images"""
        unselected_images = obj.images.filter(is_selected=False)
        return MoodboardImageSerializer(unselected_images, many=True, context=self.context).data
        ...
        if value is not None:
            ...
            for color in value:
                ...
                if not (color.startswith('#') and len(color) == 7):
                    raise serializers.ValidationError("Colors must be in hex format (#RRGGBB)")
        return value
...
class MoodboardTemplateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'title', 'description', 'template_type', 'category',
            'default_prompts', 'default_tags', 'default_color_palette',
