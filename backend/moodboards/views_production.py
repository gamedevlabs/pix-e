...
class MoodboardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .prefetch_related(
                ...
                Prefetch(
                    "comments",
                    queryset=MoodboardComment.objects.filter(is_hidden=False),
                ),
            )
            .annotate(
                total_images=Count("images", distinct=True),
                selected_images=Count(
                    "images", filter=Q(images__is_selected=True), distinct=True
                ),
        ...
        if action == "public":
            ...
            return queryset.filter(
                is_public=True, status__in=["completed", "in_progress"]
    ...
    def create(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Created new moodboard {moodboard.id} for user {request.user.username}"
        )

        # Return full detail serialization
        detail_serializer = MoodboardDetailSerializer(
    ...
    def public(self, request):
        ...
        if featured:
            ...
            queryset = queryset.filter(Q(like_count__gte=10) | Q(view_count__gte=100))
    ...
    def generate_images(self, request, pk=None):
        ...
        try:
            ...
            logger.info(
                f"Generated {len(generated_images)} images for moodboard {moodboard.id}"
            )

            # Return updated moodboard data
            serializer = MoodboardDetailSerializer(
                moodboard, context={"request": request}
            )
            return Response(serializer.data)
            ...
            logger.error(
                f"Image generation failed for moodboard {moodboard.id}: {str(e)}"
            )
            return Response(
    ...
    def bulk_image_action(self, request, pk=None):
        ...
        images = MoodboardImage.objects.filter(moodboard=moodboard, id__in=image_ids)

        if not images.exists():
            return Response(
                {"error": "No valid images found"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            updated_serializer = MoodboardDetailSerializer(
                moodboard, context={"request": request}
            )
    ...
    def bulk_action(self, request):
        ...
        moodboards = Moodboard.objects.filter(user=request.user, id__in=moodboard_ids)
        ...
        try:
            ...
            if action_type == "delete":
                moodboards.delete()
                logger.info(
                    f"Bulk deleted {count} moodboards for user {request.user.username}"
                )
            ...
            return Response(
                {"status": "success", "action": action_type, "count": count}
            )
...
class MoodboardImageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            MoodboardImage.objects.select_related("moodboard", "moodboard__user")
            .filter(
    ...
    def perform_create(self, serializer):
        ...
        if not moodboard.can_user_edit(self.request.user):
            raise PermissionDenied(
                "You don't have permission to add images to this moodboard"
            )
...
class LegacySessionView(viewsets.ViewSet):
    ...
    def start(self, request):
        ...
        moodboard = Moodboard.objects.create(
            user=request.user, title="Untitled Moodboard", status="draft"
        )

        logger.info(
            f"Legacy session compatibility: Created draft moodboard {moodboard.id}"
        )
    ...
    try:
        ...
        stats = {
            ...
            "images": {
                ...
                "ai_generated": MoodboardImage.objects.filter(
                    source="ai_generated"
                ).count(),
                "user_uploaded": MoodboardImage.objects.filter(
                    source="user_upload"
                ).count(),
            },
            "users": {
                ...
                "active_creators": User.objects.filter(
                    moodboards__created_at__gte=timezone.now().replace(
                        day=timezone.now().day - 30
                    )
                )
                .distinct()
                .count(),
